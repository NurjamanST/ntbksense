...
/**
 * =========================================================================
 * FILE: api/setting/ads.setting.php
 * FUNGSI: Menerima data dari AJAX, memproses, dan menyimpan ke tabel.
 * VERSI: Ditambahkan mode debugging untuk menangkap output error.
 * =========================================================================
 */

// [DEBUGGING] Mulai output buffering untuk menangkap semua output (termasuk error)
ob_start();
...
if (!file_exists($wp_load_path)) {
    header("Content-Type: application/json; charset=UTF-8");
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'FATAL ERROR: Gagal memuat environment WordPress. Path: ' . $wp_load_path]);
    exit;
}
require_once($wp_load_path);

/**
 * Fungsi utama untuk menangani update.
 */
function handle_ntbksense_database_settings_update()
{
    // Set header JSON di sini, SETELAH WordPress dimuat.
    header("Content-Type: application/json; charset=UTF-8");

    // 1. Keamanan: Cek nonce dan izin user.
    if (!isset($_POST['security']) || !wp_verify_nonce($_POST['security'], 'ntbksense_settings_nonce')) {
        http_response_code(403);
        return json_encode(['success' => false, 'message' => 'Validasi keamanan gagal!']);
    }
    if (!current_user_can('manage_options')) {
        http_response_code(403);
        return json_encode(['success' => false, 'message' => 'Anda tidak memiliki izin.']);
    }
    ...
    $data_to_save = [
        'id_publisher'             => sanitize_text_field($_POST['id_publisher'] ?? ''),
        'opacity'                  => absint($_POST['opacity'] ?? 100),
        'jumlah_iklan'             => absint($_POST['jumlah_iklan'] ?? 5),
        'mode_tampilan'            => sanitize_text_field($_POST['mode_tampilan'] ?? 'Berurutan'),
        'posisi'                   => sanitize_text_field($_POST['posisi'] ?? 'Fixed'),
        'margin_top'               => absint($_POST['margin_top'] ?? 5),
        'margin_bottom'            => absint($_POST['margin_bottom'] ?? 5),
        'kode_iklan_1'             => base64_encode(stripslashes($_POST['kode_iklan_1'] ?? '')),
        'kode_iklan_2'             => base64_encode(stripslashes($_POST['kode_iklan_2'] ?? '')),
        'kode_iklan_3'             => base64_encode(stripslashes($_POST['kode_iklan_3'] ?? '')),
        'kode_iklan_4'             => base64_encode(stripslashes($_POST['kode_iklan_4'] ?? '')),
        'kode_iklan_5'             => base64_encode(stripslashes($_POST['kode_iklan_5'] ?? '')),
        'refresh_blank'            => absint($_POST['refresh_blank'] ?? 0),
        'sembunyikan_vinyet'       => absint($_POST['sembunyikan_vinyet'] ?? 0),
        'sembunyikan_anchor'       => absint($_POST['sembunyikan_anchor'] ?? 0),
        'sembunyikan_elemen_blank' => absint($_POST['sembunyikan_elemen_blank'] ?? 0),
        'tampilkan_close_ads'      => absint($_POST['tampilkan_close_ads'] ?? 0),
        'auto_scroll'              => absint($_POST['auto_scroll'] ?? 0),
        // 'jenis_pengalihan'         => sanitize_text_field($_POST['jenis_pengalihan'] ?? '302 Temporary - PHP'),
        // 'akses_lokasi_mode'        => sanitize_text_field($_POST['akses_lokasi_mode'] ?? 'off'),
        // 'akses_lokasi_countries'   => isset($_POST['akses_lokasi_countries']) ? wp_json_encode(array_map('sanitize_text_field', $_POST['akses_lokasi_countries'])) : '[]',
        // 'blokir_ip_address'        => sanitize_textarea_field($_POST['blokir_ip_address'] ?? ''),
        // 'akses_asn_mode'           => sanitize_text_field($_POST['akses_asn_mode'] ?? 'off'),
        // 'akses_asn_list'           => sanitize_textarea_field($_POST['akses_asn_list'] ?? ''),
        // 'detektor_perangkat_tinggi' => absint($_POST['detektor_perangkat_tinggi'] ?? 0),
        // 'detektor_perangkat_sedang' => absint($_POST['detektor_perangkat_sedang'] ?? 0),
        // 'detektor_perangkat_rendah' => absint($_POST['detektor_perangkat_rendah'] ?? 0),
        // 'simpan_log'               => sanitize_text_field($_POST['simpan_log'] ?? 'Aktifkan'),
        // 'simpan_durasi'            => sanitize_text_field($_POST['simpan_durasi'] ?? 'Tidak (Default)'),
        // 'interval_durasi'          => absint($_POST['interval_durasi'] ?? 5),
    ];
    ...
    // 4. Kirim balasan yang lebih jelas ke JavaScript.
    if ($result === false) {
        http_response_code(500);
        return json_encode([
            'success' => false,
        ...
        http_response_code(200);
        return json_encode(['success' => true, 'message' => 'Tidak ada perubahan data yang disimpan.']);
    } else {
        http_response_code(200);
        return json_encode(['success' => true, 'message' => 'Pengaturan berhasil diperbarui.']);
    }
}

// Panggil fungsi utama dan simpan hasilnya
$json_response = handle_ntbksense_database_settings_update();

// [DEBUGGING] Ambil semua output yang mungkin nyasar (error, warning, spasi)
$stray_output = ob_get_clean();

// Jika ada output nyasar, kirim itu sebagai pesan error
if (!empty($stray_output)) {
    // Pastikan header JSON dikirim jika belum
    if (!headers_sent()) {
        header("Content-Type: application/json; charset=UTF-8");
    }
    http_response_code(500); // Internal Server Error
    // Kirim output nyasar itu sebagai pesan agar bisa dilihat di console JS
    echo json_encode([
        'success' => false,
        'message' => 'Terjadi error pada server. Respon tidak valid.',
        'debug_info' => $stray_output
    ]);
} else {
    // Jika tidak ada output nyasar, kirim response JSON yang seharusnya
    echo $json_response;
}
