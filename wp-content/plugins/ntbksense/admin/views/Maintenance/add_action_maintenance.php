...
?>
    ...
    <script>
      jQuery(document).ready(function($) {
        ...
        var maintNonce = '<?php echo wp_create_nonce("ntbksense_maintenance_nonce"); ?>';
        ...
        function showLoadingIndicator(button) {
          $(button).prop('disabled', true)
                   ...
                   .text('Loading...'); // Disable button, show loading text, change cursor
        }

        function hideLoadingIndicator(button, originalText) {
          $(button).prop('disabled', false)
                   .css('cursor', 'pointer')
                   .text(originalText); // Enable button, reset text, change cursor back
        }
        ...
        $('form[action="#ntb-geo-ip-results"]').on('submit', function(e) {
          ...
          $.post(ajaxurl, { action: 'ntbksense_geoip_lookup', security: maintNonce, ip: ip, ua: ua }, function(resp) {
            ...
            if (resp.success) {
              ...
              var resultHtml = '<pre style="white-space:pre-wrap;">' + JSON.stringify(geoInfo, null, 2) + '</pre>';
              resultHtml += '<p><strong>User Agent:</strong> ' + userAgent + '</p>';
              $('#geo-editor-container').html(resultHtml);
