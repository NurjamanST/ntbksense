            ...
            <tbody>
                <?php if (!empty($data)) : ?>
                    <?php foreach ($data as $item) : ?>
                        <tr>
                            <td><input type="checkbox" name="lp[]" value="<?php echo esc_attr($item['id']); ?>" /></td>
                            <td>
                                <div class="d-flex w-100 justify-content-start align-items-center">
                                    <img src="<?php echo esc_url(NTBKSENSE_PLUGIN_URL . 'assets/images/' . esc_attr($item['gambar'])); ?>" alt="Gambar" style="width: 50px; height: 50px; border-radius: 5px; margin-right: 10px;">
                                    <div class="d-flex flex-column">
                                        <span><?php echo esc_html($item['template']); ?></span>
                                        <small class="text-muted">(Long Screenshoot)</small>
                                        <div class="row-actions">
                                            <a style="text-decoration:none;" href="<?php echo esc_url(admin_url('admin.php?page=ntbksense-edit-lp&id=' . $item['id'])); ?>" class="text-primary">
                                                <span class="dashicons dashicons-edit"></span>
                                            </a>
                                            <a style="text-decoration:none;" href="<?php echo esc_url(admin_url('admin.php?page=ntbksense-duplicate-lp&id=' . $item['id'])); ?>" class="text-secondary">
                                                <span class="dashicons dashicons-admin-page"></span>
                                            </a>
                                            <a style="text-decoration:none;" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=ntbksense-landing-page&action=delete&id=' . $item['id']), 'ntb_delete_lp_nonce')); ?>" class="text-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus landing page ini?');">
                            ...
                            <td>
                                <div class="url-lp-input-group">
                                    <input 
                                        type="text" 
                                        value="<?php echo esc_attr($item['url_public']); ?>" 
                                        data-ads-url="<?php echo esc_attr($item['url_ads'].'$text_mode_key='.$item['text_mode_key']); ?>" 
                                        disabled 
                                        class="url-lp-input">
                                    <!-- Tombol Salin URL Public -->
                                    <button class="url-lp-action-btn copy-btn" title="Salin URL Public">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-copy"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2v2"/></svg>
                                    </button>
                                    <!-- Tombol Salin URL Adsense -->
                            ...
                            <td>
                                <div class="ntb-param-group">
                                    <input type="text" value="<?php echo esc_attr($item['parameter']); ?>" disabled class="ntb-param-input">
                            ...
                            <td>
                                <label class="ntb-switch">
                                    <input type="checkbox" 
                                        class="ntb-status-toggle" 
                                        data-id="<?php echo esc_attr($item['id']); ?>"
                                        <?php checked($item['status'], 'Aktif'); ?>>
                                    <span class="ntb-slider"></span>
                                </label>
                            </td>
                            <td><small><?php echo esc_html($item['device']); ?></small></td>
                            <td><small><?php echo esc_html($item['date']); ?></small></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
...
<style>
    ...
    /* Action Buttons Bar */
        ...
        .ntb-btn-danger { background: #dc3545 !important; border-color: #dc3545 !important; color: white !important; }
        .ntb-btn-success { background: #28a745 !important; border-color: #28a745 !important; color: white !important; }
        .ntb-btn-warning { background: #ffc107 !important; border-color: #ffc107 !important; color: #212529 !important; }
    ...
    /* Table Styling */
        ...
        #ntb-landing-page-table th, #ntb-landing-page-table td {
            padding: 8px 10px;
    ...
    /* Parameter Input Group */
        .ntb-param-group {
            ...
            gap: 2px;
            background-color: #f6f7f7; 
            border: 1px solid #cdd0d4; /* Sedikit lebih gelap agar cocok dengan gambar */
            border-radius: 6px;
            padding: 2px;
            height: 32px;
            box-shadow: inset 0 1px 2px rgba(0,0,0,.05);
        ...
        .ntb-param-copy-btn {
            background: #ffffff; /* Latar belakang putih seperti di gambar */
            border: 1px solid #cdd0d4; /* Border tipis di sekitar tombol */
            cursor: pointer;
            padding: 4px;
            display: flex;
            align-items: center;
        ...
        .ntb-param-copy-btn svg {
            width: 16px;
            height: 16px;
            color: #50575e; /* Warna ikon sedikit lebih gelap */
        }
    ...
    /* Copy Notification Styling */
        #ntb-copy-notification {
            ...
            min-width: 250px;
            background-color: #32373c; /* Warna admin WordPress */
            color: #fff;
            text-align: center;
            border-radius: 5px;
            padding: 16px;
    ...
    /* Toggle Switch Styling */
        ...
        .ntb-slider {
            ...
            transition: .4s;
            border-radius: 24px; /* Membuat ujungnya bulat */
        }

        .ntb-slider:before {
            position: absolute;
            content: "";
            height: 18px; /* Ukuran lingkaran dalam */
            width: 18px;
            left: 3px;
            bottom: 3px;
        ...
        input:checked + .ntb-slider:before {
            ...
            transform: translateX(20px); /* Jarak pergeseran lingkaran */
        }
</style>
